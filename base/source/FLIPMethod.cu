...
void FLIPMethod::initialize( const Grid& grid )
{
...
void FLIPMethod::prepareNextStep( Particles& pts )
{
    _frame->buildFromPoints( pts.position, params.enableWaterTank, _voxelWallLevel );
}

void FLIPMethod::updateParticles( Particles& pts, const float dt )
{
    ...
    auto kernel = BORA_DEVICE_KERNEL
    {
        Vec3f& pos = p_pos[ix];
        Vec3f& vel = p_vel[ix];
...
void FLIPMethod::advanceOneFrame( Particles& pts )
{
...
void FLIPMethod::advanceOneStep( Particles& pts, const float dt )
{
    ...
    auto mm_kernel = BORA_DEVICE_KERNEL
    {
        size_t i,j,k;
        ...
        if( pMass[ix] > 0.f )
        {
            pMark[ix] = FluidProjection::kFluid;
        }
        else
        {
            if( phi < 0.f ) pMark[ix] = FluidProjection::kSolid;
            else pMark[ix] = FluidProjection::kAir;
        }

        if( phi < 0.f ) pPreV[ix] += v*colVelScale;

        /* add curl */
        if( useCurl && cell.y > voxelWallLevel-2.f && pMark[ix] != FluidProjection::kSolid )
        {
    ...
    LaunchCudaDevice( mm_kernel, 0, _sMarkField.size() );
    SyncCuda();
...
void FLIPMethod::postProcess( Particles& pts )
{
    prepareNextStep( pts );
    _isPrepared = true;
    ...
    if( params.enableWaterTank == true )
    {
        ...
        LaunchCudaDevice( mm_kernel, 0, surfField.size() );
        SyncCuda();
    }
    ...
    auto vort_kernel = BORA_DEVICE_KERNEL
    {
    ...
    LaunchCudaDevice( vort_kernel, 0, pts.position.size() );
    SyncCuda();

    FieldOP::normalGradient( _sSurfField, _sCompField );
    FieldOP::curvature( _sCompField, _sPresField );

    auto curv_kernel = BORA_DEVICE_KERNEL
    {
        pCurv[ix] = presField.lerp( pPose[ix] );
    };

    LaunchCudaDevice( curv_kernel, 0, pts.position.size() );
    SyncCuda(); 
...
void FLIPMethod::fillWaterTank( Particles& particles )
{
...
void
FLIPMethod::particlesToWorld( Particles& pts )
{
    const Grid& grid = _grid;
    ...
    auto kernel_world = BORA_DEVICE_KERNEL
    {
        pos[ix] = grid.worldPoint( pos[ix] );
        vel[ix] = grid.worldVector( vel[ix] );
    };

    LaunchCudaDevice( kernel_world, 0, pts.position.size() );
    SyncCuda();    
}

void
FLIPMethod::particlesToVoxel( Particles& pts )
{
    const Grid& grid = _grid;
    ...
    auto kernel_voxel = BORA_DEVICE_KERNEL
    {
        pos[ix] = grid.voxelPoint( pos[ix] );
        vel[ix] = grid.voxelVector( vel[ix] );
    };

    LaunchCudaDevice( kernel_voxel, 0, pts.position.size() );
    SyncCuda();    
}
