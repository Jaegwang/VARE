...
/**
	This class implements a 1D array of various data types.
*/
template<class TT>
class Array
{
    protected:
        ...
        MemorySpace _memorySpace     = kUnified;        
        TT*         _data            = (TT*)0;   // the pointer of the data
        size_t      _size            = 0;        // the size of the array
        size_t      _capacity        = 0;        // the capacity of the array
        float       _increasingRatio = 1.5f;      // the increasing ratio of the array
        bool        _reference       = false;    // if true, the array is the reference of other array
    ...
    public:
        ...
        */
        VARE_UNIFIED Array( const Array<TT>& a );
        ...
        */
        Array( const size_t n, const MemorySpace memorySpace=kUnified );
        ...
        */
        ...
        /**
            ...
            @param[in] memorySpace The memory space in which it lives
        */
        void initialize( const size_t n, const MemorySpace memorySpace=kUnified );
		...
		*/
        ...
        VARE_UNIFIED const size_t num() const { return _size; }
        VARE_UNIFIED const size_t size() const { return _size; }
        VARE_UNIFIED const size_t length() const { return _size; }        

        VARE_UNIFIED const size_t capacity() const { return _capacity; }

        VARE_UNIFIED const MemorySpace memorySpace() const { return _memorySpace; }

        VARE_UNIFIED const float increasingRatio() const { return _increasingRatio; }

		VARE_UNIFIED TT* pointer() const { return _data; }
		VARE_UNIFIED TT** pPointer() { return &_data; }

		VARE_UNIFIED TT* begin() { return _data; }
		VARE_UNIFIED const TT* begin() const { return _data; }

		VARE_UNIFIED TT* end() { return ( _data + _size ); }
		VARE_UNIFIED const TT* end() const { return ( _data + _size ); }

		VARE_UNIFIED TT& at( const size_t& i ) { return _data[i]; }
		VARE_UNIFIED const TT& at( const size_t& i ) const { return _data[i]; }

		VARE_UNIFIED TT& operator[]( const size_t& i ) { return _data[i]; }
		VARE_UNIFIED const TT& operator[]( const size_t& i ) const { return _data[i]; }

		VARE_UNIFIED TT& pop_back() { return _data[--_size]; }
...
template <class TT>
VARE_UNIFIED Array<TT>::Array()
{
    // nothing to do
}

template <class TT>
VARE_UNIFIED Array<TT>::Array( const Array<TT>& a )
{
...
inline TT* Array<TT>::allocate( const size_t n )
{
    ...
    else if( _memorySpace == kDevice  ) { cudaMalloc       ( (void**)&ptr, sizeof(TT)*n ); }
    else if( _memorySpace == kUnified ) { cudaMallocManaged( (void**)&ptr, sizeof(TT)*n ); }
