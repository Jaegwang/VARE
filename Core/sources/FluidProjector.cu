...
FluidProjector::FluidProjector()
{
    _fluidCellBuffer.initialize( 0, MemorySpace::kUnified );
}

void
FluidProjector::buildLinearSystem( DenseField<size_t>& typField, const VectorDenseField& velField, const ScalarDenseField& pressField )
{
    ...
    for( int n=0; n<count; ++n )
    {
        ...
        inds[0] = t_i0 >= kFluid ? t_i0-kFluid : INVALID_MAX;
        inds[1] = t_i1 >= kFluid ? t_i1-kFluid : INVALID_MAX;
        inds[2] = t_j0 >= kFluid ? t_j0-kFluid : INVALID_MAX;
        inds[3] = t_j1 >= kFluid ? t_j1-kFluid : INVALID_MAX;
        inds[4] = t_k0 >= kFluid ? t_k0-kFluid : INVALID_MAX;
        inds[5] = t_k1 >= kFluid ? t_k1-kFluid : INVALID_MAX;
        inds[6] = t_cc-kFluid;
...
int
FluidProjector::solve( VectorDenseField& velField, ScalarDenseField& _pressureField, DenseField<size_t>& typField )
{
    /* Solve linear system */
...
void
FluidProjector::buildLinearSystem( IdxSparseField& typField, const Vec3fSparseField& veloField, const FloatSparseField& pressField )
{
    ...
    {
        size_t i,j,k;
        typField.findIndices( fluidCellBuffer[ix], i,j,k );
        ...
        inds[0] = t_cc-kFluid;
        inds[1] = t_i0 >= kFluid ? t_i0-kFluid : INVALID_MAX;
        inds[2] = t_i1 >= kFluid ? t_i1-kFluid : INVALID_MAX;
        inds[3] = t_j0 >= kFluid ? t_j0-kFluid : INVALID_MAX;
        inds[4] = t_j1 >= kFluid ? t_j1-kFluid : INVALID_MAX;
        inds[5] = t_k0 >= kFluid ? t_k0-kFluid : INVALID_MAX;
        inds[6] = t_k1 >= kFluid ? t_k1-kFluid : INVALID_MAX;

        px[ix] = pressField(i,j,k);
        pb[ix] = d;
    };

    LaunchDeviceKernel( kernel, 0, count );
    SyncKernels();
...
int
FluidProjector::solve( Vec3fSparseField& veloField, FloatSparseField& pressField, const IdxSparseField& typField )
{
    /* Solve linear-system */
    int iternum(0);
/*    
...
*/
    ...
    auto kernel_press = VARE_DEVICE_KERNEL
    {
        size_t idx = fluidCellBuffer[ix];
    ...
    LaunchDeviceKernel( kernel_press, 0, fluidCellBuffer.size() );
    SyncKernels();
    ...
    auto kernel_neig_press = VARE_DEVICE_KERNEL
    {
        if( typField[ix] == kAir )
        {
            size_t i,j,k;
            pressField.findIndices( ix, i,j,k );
            ...
            if( count > 0 ) pPress[ix] = press/(float)count;
        }
    };

    LaunchDeviceKernel( kernel_neig_press, 0, typField.size() );
    SyncKernels();
    ...
    LaunchDeviceKernel( kernel, 0, fluidCellBuffer.size() );
    SyncKernels();
