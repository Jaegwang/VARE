...
FieldOP::gradient( const ScalarDenseField& src, VectorDenseField& dst )
{
    ...
    {
        size_t i,j,k;
        src.cellIndices( ix, i,j,k );

        const float& v = pSrc[ix];
        ...
        pDst[ix] = Vec3f( (v_i1-v_i0)*0.5f,
                          (v_j1-v_j0)*0.5f,
                          (v_k1-v_k0)*0.5f );
    };

    LaunchDeviceKernel( kernel, 0, src.size() );
    SyncKernels();
}
...
FieldOP::curvature( const ScalarDenseField& surf, ScalarDenseField& curv )
{
    ...
    auto kernel = VARE_DEVICE_KERNEL
    {
        ...
        pCurv[ix] = (6.f*s -s_i0-s_i1-s_j0-s_j1-s_k0-s_k1);
    };
...
FieldOP::curvature( const VectorDenseField& norm, ScalarDenseField& curv )
{
    ...
    {
        size_t i,j,k;
        norm.cellIndices( ix, i,j,k );

        const Vec3f& s = pNorm[ix];
        ...
        pCurv[ix] = (s_i1-s_i0)*0.5f + (s_j1-s_j0)*0.5f + (s_k1-s_k0)*0.5f;
    };

    LaunchDeviceKernel( kernel, 0, norm.size() );
    SyncKernels();
}
...
FieldOP::curl( const VectorDenseField& vel, VectorDenseField& cur )
{
    ...
    {
        size_t i,j,k;
        vel.cellIndices( ix, i,j,k );

        const Vec3f& v = vel[ix];
    ...
    LaunchDeviceKernel( kernel, 0, cur.size() );
    SyncKernels();
}
